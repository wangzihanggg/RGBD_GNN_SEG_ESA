...
def getHierarchy(slicmap):
    ...
    '''
    segmentmap = slicmap
    layers, h, w = segmentmap.shape
    segs = np.concatenate([np.reshape([i for i in range(h * w)], [1, h, w]), segmentmap], axis=0)
    layers = layers + 1
